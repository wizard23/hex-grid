...
import { CENTRE, createModel, setTriangleState, toggleEdge, toggleSpoke, toggleVertex } from "./model";
...
test("a document survives a save/load round trip", () => {
  ...
  const model = setTriangleState(
    toggleVertex(toggleEdge(toggleSpoke(createModel(3, 2), 2, 1, 5), 0, 0, 2), 1, 1, CENTRE),
    2,
    1,
    4,
    2,
  ...
  assert.deepEqual([...loaded.model.triangles], [...model.triangles]);
});

test("version 3 files (no fills) load with every triangle at state 0; states are capped on load", () => {
  const v3 = parseDocument(
    '{"format":"hex-grid","version":3,"settings":{"columns":1,"rows":1},"spokes":[1],"edges":[3],"vertices":[4]}',
  );
  assert.deepEqual([...v3.model.triangles], [0, 0, 0, 0, 0, 0]);
  assert.equal(v3.settings.stateCount, DEFAULT_SETTINGS.stateCount);
  const capped = parseDocument(
    '{"format":"hex-grid","version":4,"settings":{"columns":1,"rows":1,"stateCount":2},"spokes":[1],"edges":[3],"vertices":[4],"triangles":[0,1,2,9,0,1]}',
  );
  assert.deepEqual([...capped.model.triangles], [0, 1, 1, 1, 0, 1]);
});
...
test("parseDocument rejects foreign or corrupt input with a readable message", () => {
  ...
  assert.throws(() => parseDocument('{"format":"hex-grid","version":5}'), /unsupported version/);
  assert.throws(
  ...
  assert.throws(
    () =>
      parseDocument(
        '{"format":"hex-grid","version":4,"settings":{"columns":1,"rows":1},"spokes":[1],"edges":[1],"vertices":[1],"triangles":[1]}',
      ),
    /expected 6 triangle bytes/,
  );
});
...
test("parseSettings clamps numbers and falls back to defaults", () => {
  ...
  const colors = parseSettings({ stateCount: 99, stateColors: ["#111111", "nope"] });
  assert.equal(colors.stateCount, 10);
  assert.equal(colors.stateColors.length, DEFAULT_SETTINGS.stateColors.length);
  assert.equal(colors.stateColors[0], "#111111");
  assert.equal(colors.stateColors[1], DEFAULT_SETTINGS.stateColors[1]);
  assert.equal(parseSettings({ stateCount: 1 }).stateCount, 2);
